...
async def on_message(message: Message):
    ...
    if message.text == "آن میوت کردن" :
    ...
    elif message.chat_id in mutegr :
         await bot.delete_message(chat_id=message.chat_id,message_id=message.message_id)
         return()
        ...
        if not message.author.id == 1366209144 :
            ...
            if "گروه باحالان" in msg :
                ...
                print("sended")
    elif "حساب کردن " in message.text :
    ...
    elif message.text=="میوت کردن گروه":
         if admin == True :
    ...
    elif message.text == "راهنما" or message.text == "دستورات" :
          await message.reply("لیست دستورات ربات 📚 : \n  - میوت کردن گروه \n  - آن میوت کردن \n  - بن (پاسخ به پیام فرد مورد نظر) \n  - فال حافظ \n  - قیمت دلار \n  - قیمت سکه \n  - سخن بزرگان \n  - دانستنی \n  - ساعت \n  - تاريخ")

    else:
         c=message.text.replace("+","").replace("-","").replace("*","").replace("/","")
         if numch(c) == True :
              await message.reply(str(eval(message.text)))
